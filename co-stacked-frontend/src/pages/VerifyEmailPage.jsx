...
import { verifyEmail } from '../features/auth/authSlice'; // We will create this action next
...
import styles from './LoginPage.module.css'; // Reuse login page styles
...
export const VerifyEmailPage = () => {
  ...
  // registered, redirect them to the signup page.
  useEffect(() => {
    if (!unverifiedEmail) {
      navigate('/signup');
    }
  ...
  const handleSubmit = async (e) => {
    ...
    if (verifyEmail.fulfilled.match(resultAction)) {
      // On success, wait a moment so the user can read the success message, then redirect
      setTimeout(() => {
        navigate('/login');
      }, 2500);
    }
